...
import type { TouchTexture, TouchPoint } from './pixelBlastTypes';
...
export const createTouchTexture = (): TouchTexture => {
	...
	const drawPoint = (p: TouchPoint) => {
		...
		// Ramps up over the first 30% of the point's life, then decays over the rest.
		const falloff =
			p.age < maxAge * 0.3
				? easeOutSine(p.age / (maxAge * 0.3))
				: easeOutQuad(1 - (p.age - maxAge * 0.3) / (maxAge * 0.7)) || 0;

		const intensity = falloff * p.force;
